...
api.put '/users/:id' do |input, req|
  [{ updated: true }, 200]
end

api.patch '/users/:id' do |input, req|
  [{ patched: true }, 200]
end

api.delete '/users/:id' do |input, req|
  [{}, 204]
end
```
...
api.get '/posts/:post_id/comments/:comment_id' do |input, req|
...
api.get '/search', query: SearchSchema do |input, req|
...
UsersRouter = FunApi::Router.new(prefix: '/users', tags: ['users']) do |r|
  ...
  r.get '/:id', path: IdSchema do |input, req|
...
UsersRouter = FunApi::Router.new(prefix: '/users', depends: { db: :db }) do |r|
  # `db` is injected here
  r.get '/' do |input, req, db:|
    [db.all_users, 200]
  end

  # route-level `store` overrides any shared `store`
  r.get '/audit', depends: { store: :audit_store } do |input, req, db:, store:|
    [store.recent, 200]
  end
end
